...
- [~] **EXP-006** — **Tool-using agent loop** (Claude Code / Codex shape). Replaces the 4-stage code-exec orchestrator with a tool-using loop where the agent inspects/fits/verifies incrementally instead of writing one ~400-line generation script in one shot. Design: `2026-05-07-tool-using-agent-design.md`. **Subsumes EXP-002 (verify-family thresholds), EXP-003 (`fit_conditional(allow_missing=True)`), and EXP-004 (system prompt + tool guidance, not stage-prompt branching).** *Expected lift:* +0.05–0.15 mostly by recovering the cells that crashed under EXP-001 + letting the agent verify before it commits. *Cost:* multi-day rewrite of orchestrator + new tools module; spike on `acs` first.
  - **Stage A (ACS spike, 2026-05-07):** ✅ done. 0.498 overall (vs baseline 0.505 = -0.007 within noise; vs `exp001_rubric_cross[acs]` 0.369 = +0.129). T1 +0.146 / T2 +0.030 / T3 -0.197 vs baseline; T3 +0.143 vs rubric. 19 turns, 69 tool calls, 9 replace_step (verify-then-revise loop fired), no crashes. See `2026-05-07-exp006a_tools_acs-report.md`.
  - **Stage B (all 7 datasets, 2026-05-07):** ✅ done. 7/7 ran end-to-end with no crashes — including gss/cps/addhealth which had crashed under EXP-001. **Beat paper-best on overall in 7/7 datasets** (+0.002 to +0.199). **vs same-model baseline (legacy code-block):** mixed — gss +0.086, cps -0.079, acs -0.102, nlsy ≈ tied, addhealth -0.057, cfps ≈ tied. T3 is the dominant gap; agent overuses `empirical_lookup` for numeric targets where `linear_regression` would preserve residual structure. ACS rerun (Stage B) scored 0.402 vs Stage A's 0.498 — gpt-5.4 stochasticity is large (~0.10 on overall). See `2026-05-07-exp006b_tools_cross-report.md` and `..._long-report.md`.
...
- [ ] **EXP-006d** — Tools-path ablation (same `rubric_tools_v2` prompt, cross datasets only, full_agent + agent_no_semantic + agent_no_data). Tests whether the EXP-001 finding "agent_no_semantic outscored full_agent on ACS by +0.27" persists when the agent uses tools. Yaml entry `exp006d_tools_ablation_cross` ready; gate on EXP-006c results before running.
