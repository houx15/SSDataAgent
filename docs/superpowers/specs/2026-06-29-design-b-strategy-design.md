...
- **LLM = marginals knob; copula = data-grounded where signed data exists.** The LLM elicits per-cell per-target conditional distributions; cross-target correlation comes from **signed microdata** correlations: train (A), source (B). In **C the copula is the identity** (independence) — `known_associations` are *unsigned* magnitudes (Cramér's V / |r| / η), which cannot define a signed Gaussian copula, so C "shrinks fully toward independence" (design-reference's stated default) and its grounding lives entirely in the marginals. `known_associations` stays exposed (3a) for diagnostics/future designs but is not consumed by Design B v1. The LLM never proposes the copula.
- **Uniform target representation:** every target is a **probability vector over a fixed support** — categorical/ordinal → `allowed_values`; numeric → `K` even-width bins over `numeric_ranges` (default `K=10`). The LLM emits these vectors, raking (IPF) calibrates them to the known (binned) marginal, and sampling draws a support index then, for numeric targets, a value uniformly within the chosen bin. One code path for both types.
...
New files:
- `src/ssdataagent/strategies/copula.py` — the Gaussian-copula latent machinery, **extracted** from `baselines.py` (no behavior change): `build_cuts`, `latent_value`, `latent_matrix`, `invert`, `make_pd`, plus an unconditional `correlated_normal(Sigma, n_samples, rng)` helper (the draw Design B v1 uses). `baselines.py` re-imports these (its `copula_generate` keeps working unchanged). A `conditional_gaussian_sample` helper is **not** added this cycle — the background-conditioned copula is out of scope, so its conditional draw is deferred (YAGNI).
...
For each cell, one `client.chat` call. The prompt carries: the population context, the cell's demographic description (the coarsened background values), the target variables with descriptions + `allowed_values`/`numeric_ranges`, the **known overall marginals** (as anchors), and — in condition B — an explicit *transport* instruction (these source-population shapes must be adapted to the target context). It requests strict JSON: per target, a **probability vector over that target's support** (categorical → `allowed_values`; numeric → the `K` even-width bins, presented as ranges), summing ~1, explicitly prompted for realistic within-group **spread** (counter the modal collapse).

- **Parse + validate:** each vector is renormalized over the target's support (length-checked, non-negative). Malformed JSON → up to N retries (default 3); final fallback = the known (binned) marginal for that target (a safe, grounded default) with the failure logged.
...
1. **Reference data.** `ref = gate.fit_microdata()` (train in A, source[crosswalk] in B) or `None` in C; `known_m = gate.known_marginals()` (raking anchor). Determine the **target set**: schema targets present in `known_m` (= crosswalk targets under B). (`known_associations` is not consumed by v1 — C uses an identity copula.)
...
5. **Couple.** Build the copula correlation matrix over the target latent space: from **signed microdata correlations** when `ref` exists — A/B — via `copula.build_cuts` + empirical correlation; **identity** in C (independence, since `known_associations` are unsigned). One shared copula (cells differ in marginals, not dependence) keeps it tractable.
...
- `tests/test_strategy_design_b.py` — with a mocked client returning fixed per-cell distributions: raking makes the cell-weighted marginal match `known_marginals` (within tol); output respects `allowed_values`/`numeric_ranges`; deterministic across two runs (cache); **no leakage in B** (only target-set/crosswalk targets filled by the model); `fit_microdata() is None` in C still works (uses `known_marginals` for raking + identity copula).
...
- **C copula is identity (independence).** `known_associations` are unsigned, so v1 fabricates no signs — C's dependence is conservatively independent and its grounding is the marginals. Signed dependence in C (e.g. a gate-exposed train correlation) is future work; flagged so the C Type-2/3 numbers are read as a floor, not Design B's best.
